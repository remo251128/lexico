...
const AudioManager = {
    ...
    init() {
      Object.values(this.sounds).forEach(sound => {
        sound.volume = 0.7; // Set default volume (0.7 = 70%)
    ...
    play(soundName) {
      try {
        this.sounds[soundName].currentTime = 0; // Rewind if already playing
...
function generateShareText() {
    ...
    let result = `Lexico LATINO ${countryName} ${guessCount}/6\n\n`;
...
function submitGuess() {
    ...
    // Only play sound AFTER validating the word
    if (!isValidWord(currentGuess)) {
        showMessage('Palabra no válida');
        shakeRow(guesses.length);
        return; // Exit before playing sound
    }
    
    // Sound plays only for valid submissions
    AudioManager.play('guess');
...
function gameWon() {
    ...
    // Update modal
    document.getElementById('game-over-title').textContent = '¡Ganaste!';
    document.getElementById('game-over-message').innerHTML = 
        `${COUNTRY_MESSAGES[currentCountry].win} <span id="correct-word">${word}</span>`;
    ...
    AudioManager.play('win'); // Play win sound
...
function gameLost() {
    ...
    // Update modal
    document.getElementById('game-over-title').textContent = '¡Perdiste!';
    document.getElementById('game-over-message').innerHTML = 
        `${COUNTRY_MESSAGES[currentCountry].lose} <span id="correct-word">${word}</span>`;
    ...
    AudioManager.play('loss'); // Play loss sound
    setTimeout(() => gameOverModal.style.display = 'flex', 1000);
}
